...
const downloadCSV = (headers: string[], data: (string | number | undefined)[][], filename: string) => {
    const csvRows = [
        headers.join(','),
        ...data.map(row => 
            row.map(field => {
...
const Finance: React.FC<FinanceProps> = ({ transactions, setTransactions, pockets, setPockets, projects, profile, cards, setCards, teamMembers, rewardLedgerEntries }) => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
            ...
            if (mode === 'add') {
                 if (newTx.type === TransactionType.EXPENSE) {
                    ...
                    if (source.startsWith('pocket-')) {
                        ...
                        // NOTE: LOGIC UPDATED - The card balance is reduced when spending from pocket
                        if (pocket.sourceCardId) {
                            const sourceCard = cards.find(c => c.id === pocket.sourceCardId);
                            if (!sourceCard || sourceCard.balance < newTx.amount) { alert(`Saldo di kartu sumber (${sourceCard?.bankName}) yang terhubung ke kantong ini tidak mencukupi.`); return; }
                ...
                setTransactions(prev => [...prev, { ...newTx, id: `TRN${Date.now()}` }].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
                 ...
                 setTransactions(prev => prev.map(t => t.id === data.id ? { ...t, ...newTx } : t).sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
        ...
        if (type === 'transfer') {
            ...
            if (isDeposit) {
                // Moving from Card to Pocket (Allocation)
                // This is a virtual allocation, card's real balance does not change.
                setPockets(prevPockets => prevPockets.map(p =>
                    p.id === pocket.id ? { ...p, amount: p.amount + amount, sourceCardId: card.id } : p
                ));
            } else { // Withdraw
                // Moving from Pocket to Card (De-allocation)
                ...
                // Card's real balance does not change, funds are just moved from 'earmarked' to 'available'.
            }
